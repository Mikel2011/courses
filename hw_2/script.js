...
btnSubmit.addEventListener("click", () => {
  inputs.forEach(input => {
    ...
    if (
      input.dataset.name === "password" ||
      input.dataset.name === "confirm_password"
    ) {
      const password = document.querySelector("input[data-name=password]");
      const confirm_password = document.querySelector(
        "input[data-name=confirm_password]"
      );
      if (
        validationObj["password"].test(password.value) &&
        validationObj["confirm_password"].test(confirm_password.value)
      ) {
